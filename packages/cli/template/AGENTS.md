...
- **Resolving "this page" / "this element"** — when the user references the current slide or selection without naming it, consult the `current-slide` skill. It reads the dev server's `node_modules/.open-frame/current.json` to find which slide, page, and inspector-picked element they mean.
