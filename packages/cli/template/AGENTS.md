...
- **Resolving "this page" / "this element"** — when the user references the current slide or selection without naming it, consult the `current-slide` skill. It reads the dev server's `node_modules/.open-studio/current.json` to find which slide, page, and inspector-picked element they mean.
...
The skills above are managed by `@open-studio/core`. Do not edit them in place. To pull the latest versions:

```
pnpm up @open-studio/core
pnpm sync:skills
```

`pnpm dev` will also detect drift on startup and offer to sync. `pnpm sync:skills --dry-run` (via `pnpm exec open-studio sync:skills --dry-run`) previews changes without writing.
